...
async function loginViaGoogle(req,res){
   let {userViaGoogle}=req.body;
   //let {email,name,picture}=userViaGoogle;

   let user= await User.findOne({email:userViaGoogle.email});
   if(!user){
    let userDoc = await User.create(userViaGoogle);
    console.log(userDoc);
